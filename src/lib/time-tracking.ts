...
export function calculateTaskMetrics(task: Task): TaskMetrics | null {
  ...
  const accuracy = timeSpent > 0 
    ? Math.max(0, 100 - Math.abs((timeSpent - estimate) / estimate) * 100)
    : 0
...
export function calculateSprintMetrics(tasks: Task[], startDate: Date, endDate: Date): SprintMetrics {
...
export function getVelocityTrend(tasks: Task[], weeks: number = 4): Array<{
  week: string
  velocity: number
