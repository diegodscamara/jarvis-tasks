...
import { addDays, startOfWeek, endOfWeek, parse, isValid } from 'date-fns'
...
export function parseNaturalLanguage(input: string): ParsedCommand {
  ...
  if (normalizedInput.includes('create') || normalizedInput.includes('add') || 
      normalizedInput.includes(':') || !normalizedInput.includes('task')) {
    return parseCreateCommand(input)
  } else if (normalizedInput.includes('mark') || normalizedInput.includes('update') ||
             normalizedInput.includes('change') || normalizedInput.includes('set')) {
    return parseUpdateCommand(input)
  } else if (normalizedInput.includes('show') || normalizedInput.includes('list') ||
             normalizedInput.includes('find') || normalizedInput.includes('get')) {
...
function isComplexTask(title: string): boolean {
  const complexKeywords = [
    ...
    'complete', 'full', 'entire', 'platform', 'system'
  ]
  const titleLower = title.toLowerCase()
  return complexKeywords.some(keyword => titleLower.includes(keyword))
}

function generateTaskBreakdown(title: string, description?: string): Array<{ title: string; description?: string }> {
  const titleLower = title.toLowerCase()
  const breakdown = []
  ...
  if (titleLower.includes('e-commerce') || titleLower.includes('online store')) {
    breakdown.push(
      ...
      { title: 'Implement shopping cart', description: 'Add cart functionality with session management' },
      { title: 'Create checkout flow', description: 'Build payment integration and order processing' },
      { title: 'Add user accounts', description: 'Implement customer registration and login' },
      { title: 'Build admin dashboard', description: 'Create product and order management interface' }
    )
    ...
    breakdown.push(
      ...
      { title: 'Create CRUD operations', description: 'Build create, read, update, delete endpoints' },
...
function findSimilarTasks(newTask: Partial<Task>, existingTasks: Task[]): Task[] {
  ...
  return existingTasks
    ...
    .map(task => ({
      task,
      score: calculateSimilarity(keywords, extractKeywords(task.title + ' ' + (task.description || '')))
    }))
...
function extractKeywords(text: string): string[] {
  const stopWords = ['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'with', 'as', 'by']
  return text
    .toLowerCase()
    .split(/\W+/)
