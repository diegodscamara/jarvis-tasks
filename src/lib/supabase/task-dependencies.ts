...
export async function validateDependency(taskId: string, newDependsOnId: string): Promise<DependencyValidation> {
  const supabase = await createSupabaseServerClient()
...
export async function calculateTaskDepth(taskId: string, memo = new Map<string, number>()): Promise<number> {
  ...
  const depths = await Promise.all(
    dependencies.map(depId => calculateTaskDepth(depId, memo))
  )
